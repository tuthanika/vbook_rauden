...
if (!empty($genre_item)) {
    $item_list = array();
    $link_url1 = "https://www.uukanshu.com" . $genre_item . "-" . $page_show . ".html";
    $res_html = curl_normal($link_url1);
    // create a new DOM object from the string
    $html = str_get_html($res_html, false);

    $li_elements = $html->find('.content > .list-item');

    foreach ($li_elements as $li) {
        $book_name = $li->find('.book-info > h3', 0)->plaintext;
        $book_id = $li->find('.book-info a', 0)->href;
        preg_match('/\/b\/(\d+)/', $book_id, $matches);
        $book_id = $matches[1];
        ...
        $author = $li->find('dl > .book-item > a', 0)->plaintext;
